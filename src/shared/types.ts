...
export interface PokerNotesAPI {
  ...
  getVersion(): Promise<string>; getSettings(): Promise<Settings>; setSettings(settings: Settings): Promise<Settings>; backupNow(): Promise<string>; hide(): Promise<void>; minimize(): Promise<void>; quit(): Promise<void>; onFocusSearch(callback: () => void): () => void; onBeforeHide(callback: () => void): () => void; onRequestQuit(callback: () => void): () => void
}
